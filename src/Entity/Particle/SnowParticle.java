...
public class SnowParticle extends Particle {
	...
	public SnowParticle(int x, int y, int life) {
		super(x, y, life);
		sprite = Sprite.particle_snow;
		this.x = x + random.nextInt(2560);
		this.y = y - random.nextInt(500);
		this.xx = x;
		this.yy = y;
		this.life = life + (random.nextInt(20) - 10);
		this.xa = 1 * random.nextGaussian();
		this.ya = .5 + 2.5 * random.nextGaussian();
		if(ya < 0) ya = -ya;
		this.zz = 0.0;
	}
